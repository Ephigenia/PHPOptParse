...
/**
 *	Automatic Test Case for {@link OptParse}
 * 	This will only be reached if you execute this file directly, otherwise
 * 	this part of the code will be ignored
 */
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
	class OptParserTest extends OptParse{
		public $config = array(
			array(
				'short' => 'h',
				'long' 	=> 'help',
			...
			array(
				'short' => 'q',
				'help' => 'sets verbose mode to not-verbose',
				'dest' => 'verbose',
				'type' => OptParse::TYPE_BOOL,
				'store' => false
			),
			array(
				'short' => 'v',
				...
				'dest'	=> 'verbose',
				'type'	=> OptParse::TYPE_BOOL,
				'help' => 'sets verbose mode to true',
				'store' => true
			)
		);
	}
	$parser = new OptParserTest();
	$parser->parse();
	echo '------------------------'.LF.'generated usage message:'.LF.'------------------------'.LF;
	echo $parser->usage().LF;
	echo '------------------------'.LF.'parsed options:'.LF.'------------------------'.LF;
	var_dump($parser->options);
	echo '------------------------'.LF.'parsed arguments:'.LF.'------------------------'.LF;
	var_dump($parser->arguments);
	exit;
}
?>